...
Calendar.prototype.show = function() {
	...
	ggWinContent += ("<TABLE WIDTH='100%' BORDER=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR='#E0EDDA' style='font-size:" + fontsize + "pt;'><TR><TD ALIGN=center>");
	ggWinContent += ("[<A HREF=\"javascript:void(0);\" " +
		...
		");" +
		"\">"+(parseInt(this.gYear)-1) +"年<\/A>]</TD><TD ALIGN=center>");
	ggWinContent += ("[<A HREF=\"javascript:void(0);\" " +
		...
		");" +
		"\">"+(prevMM+1)+"月<<\/A>]</TD><TD ALIGN=center>");
	...
	ggWinContent += ("[<A HREF=\"javascript:void(0);\" " +
		...
		");" +
		"\">"+(nextMM +1)+"月><\/A>]</TD><TD ALIGN=center>");
	ggWinContent += ("[<A HREF=\"javascript:void(0);\" " +
		...
		"'" + this.gReturnItem + "', '" + this.gMonth + "', '" + (parseInt(this.gYear)+1) + "', '" + this.gFormat + "'" +
		");" +
		"\">"+(parseInt(this.gYear)+1) +"年<\/A>]</TD></TR></TABLE><BR>");
...
Calendar.prototype.showY = function() {
	...
	ggWinContent += ("<TABLE WIDTH='100%' BORDER=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR='#c8e6b3' style='font-size:" + fontsize + "pt;'><TR><TD ALIGN=center>");
	ggWinContent += ("[<A HREF=\"javascript:void(0);\" " +
...
Calendar.prototype.cal_header = function() {
	...
	vCode = vCode + "<TD WIDTH='14%'><FONT FACE='" + fontface + "' COLOR='" + this.gHeaderColor + "'><B>　六</B></FONT></TD>";
	vCode = vCode + "</TR>";
	...
	for (j=vFirstDay; j<7; j++) {
		vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + "><FONT FACE='" + fontface + "'>" + 
			"<A HREF='javascript:void(0);' " + 
				"onMouseOver=\"window.status='日期: " + this.format_data(vDay) + "'; return true;\" " +
				"onMouseOut=\"window.status=' '; return true;\" " +
...
function Build(p_item, p_month, p_year, p_format) {
	...
	if (ggPosX == -1 && ggPosY == -1) {
		overlib(ggWinContent, AUTOSTATUSCAP, STICKY, CLOSECLICK, CSSSTYLE,
			...
			CAPTION, "选择一个日期：", OFFSETX, 20, OFFSETY, -20);
		...
		overlib(ggWinContent, AUTOSTATUSCAP, STICKY, CLOSECLICK, CSSSTYLE,
			TEXTSIZEUNIT, "pt", TEXTSIZE, 8, CAPTIONSIZEUNIT, "pt", CAPTIONSIZE, 8, CLOSESIZEUNIT, "pt", CLOSESIZE, 8,
			CAPTION, "选择一个日期：", FIXX, ggPosX, FIXY, ggPosY);
		}
