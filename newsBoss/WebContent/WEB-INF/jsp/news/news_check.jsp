...
{ 
 ...
 if (subcat2[i][0] == locationid) //[0] [1] 第一列 第二列 
	...
	<body>
		<form id="form1" name="form1" method="post" action="newsManageAction.do?action=toCheckPage">
			...
			<table width="95%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
              <tr>
                <td bgcolor="#FFFFFF" class="title14"> 查询条件：一级菜单
                  <select name="q1" id="q1" class="form12"  onchange="changeselect1(this.value);">
                      <option value="-1">==全部==</option>
                      <c:forEach var="lmFir" items="${firstList}"> <option 
                          <c:if test="${pageList.q1==lmFir.id}">selected</c:if>
                         value="${lmFir.id}"> ${lmFir.name}
                        </option>
                      </c:forEach>
                    </select>
                  二级菜单
                  <select id="q2" name="q2" class="form12" >
                    <option value="-1">==全部==</option>
                  </select>
                  专题新闻
                  <select name="q3" id="q3"  class="form12"  onChange="changeselect2(this.value);" >
                    <OPTION value="-1">==全部==</OPTION>
                    <c:forEach  items="${firstList2}" var="b"><option 
            <c:if test="${pageList.q3==b.id}">selected</c:if>
                       value="${b.id}">${b.name}
                      </option>
                    </c:forEach>
                  </select>
                  专题小类
                  <select name="q10" id="q10"  class="form12"  >
                    <option value="-1">==全部==</option>
                  </select>
                          </tr>
              <tr>
                <td bgcolor="#FFFFFF"   class="title14" style="padding-left:72px"><span >起始日期
                  <input name="q4" type="text" class="form12" id="q4" size="10" value="${pageList.q4}" readonly="true" />
                      <%=util.showDateSel("form1.q4")%> 终止日期
                  <input name="q5" type="text" class="form12" id="q5" size="10" value="${pageList.q5}"  onClick="" readonly="true"  />
              ...
              <tr>
                <td bgcolor="#FFFFFF"   class="title14" style="padding-left:72px">中文新闻
                  <select   id="q6" name="q6" class="form12" >
                      <option value="-1" 
                    <c:if test="${pageList.q6=='-1'}">selected</c:if>
                    >--全部--
                    </option>
                      <option value="0"  
                    <c:if test="${pageList.q6=='0'}">selected</c:if>
                    >中文新闻
                    </option>
                      <option value="1"  
                    <c:if test="${pageList.q6=='1'}">selected</c:if>
                  ...
                  <select  id="q7" name="q7"  class="form12"  >
                    <option value="-1"  
                    <c:if test="${pageList.q7=='-1'}">selected</c:if>
                    >--全部--
                    </option>
                    <option value="1"  
                    <c:if test="${pageList.q7=='1'}">selected</c:if>
                    ...
                    <option value="0"  
                    <c:if test="${pageList.q7=='0'}">selected</c:if>
                    >未审核
                    </option>
                  </select>
                  图片新闻
                  <select   id="q8" name="q8" class="form12"  >
                    <option value="-1"  
                    <c:if test="${pageList.q8=='-1'}">selected</c:if>
                    >--全部--
                    </option>
                    <option value="1"  
                    <c:if test="${pageList.q8=='1'}">selected</c:if>
                    ...
                    <option value="0"  
                    <c:if test="${pageList.q8=='0'}">selected</c:if>
                    >文字新闻
                    </option>
                  </select>
                  新闻作者
...
下一页 最后页
			...
			<table width="95%" border="0" cellspacing="1" bgcolor="#CCCCCC">
				<tr align="center" class="text14wb">
					...
					<td  width="8%" background="image/bg/table_head.gif">一级菜单</td>
				...
				<c:forEach var="n" items="${list}">
					...
					<tr align="center" bgcolor="#FFFFFF" class="text12">
						...
						<td>
							<a href="newsManageAction.do?action=toViewPage&id=${n.id}"><font
								color="#33CCFF">查看</font></a> | 
                                <a href="newsAction.do?action=delete&id=${n.id}"
								OnClick="return del()"><font color="#33CCFF">删除</font>	</a> | 
	     <c:if test="${n.newsCheck=='1'}">
	      <img src="image\icon\form\icon_right.gif"/></c:if>
	    <c:if test="${n.newsCheck=='0'}">
	    <a href="newsManageAction.do?action=tohavechecked&id=${n.id}">
	    <font color="#33CCFF">审核</font>
							</a>
