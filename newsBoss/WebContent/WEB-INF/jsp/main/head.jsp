  ...
  <TR>
    ...
    <TD width="779" bgColor="#ffffff">
      <TABLE cellSpacing=0 cellPadding=0 width="779" border=0>
        ...
        <TR>
          <TD width=422><a href="mainAction.do?action=toIndexPage" ><img src="image/main/logo1112.jpg" width="422" height="100" border="0" /></a></TD>
...
<div id=menu>
        ...
        <ul id="navigation"> 
            ...
            <li><a href="mainAction.do?action=toIndexPage" >首页
     ...
     <c:if test="${mainnode.type!='custom'}">
	 	<!-- Navigation item --> 
         <li><a href="mainAction.do?action=toSecondaryPage&pnid=<c:out value = '${mainnode.id}' />&nid=-2" ><c:out value = "${mainnode.name}" /></a>   </li> 
     </c:if>
     <c:if test="${mainnode.type=='custom'}">
	 	<!-- Navigation item --> 
         <li><a href="mainAction.do?action=toFacilityPage&pnid=<c:out value = '${mainnode.id}' />&nid=-2" ><c:out value = "${mainnode.name}" /></a>   </li> 
     </c:if>
	 </c:when>
	 <c:otherwise>
      <c:if test="${mainnode.type!='custom'}">
	 	 <!-- Navigation item --> 
            <li><a href="mainAction.do?action=toSecondaryPage&pnid=<c:out value = '${mainnode.id}' />&nid=-1" ><c:out value = "${mainnode.name}" />
			 <!--[if IE 7]><!-->
			 </a>
			 <!--<![endif]-->
			  <table><tr><td>
           ...
           <c:if test="${mainnode.type=='custom'}">
	 	 <!-- Navigation item --> 
            <li><a href="mainAction.do?action=toFacilityPage&pnid=<c:out value = '${mainnode.id}' />&nid=-1" ><c:out value = "${mainnode.name}" />
			 <!--[if IE 7]><!-->
			 </a>
			 <!--<![endif]-->
			  <table><tr><td>
          ...
          <c:if test="${node.type=='custom'}">
          	 <li><a href="mainAction.do?action=toFacilityPage&pnid=<c:out value = '${mainnode.id}' />&nid=<c:out value = '${node.id}' />&q1=${node.boFacilityType.id}" ><c:out value="${node.name}" /></a></li> 
          </c:if>
          <c:if test="${node.type!='custom'}">
          	 <li><a href="mainAction.do?action=toSecondaryPage&pnid=<c:out value = '${mainnode.id}' />&nid=<c:out value = '${node.id}' />"  ><c:out value="${node.name}" /></a></li> 
          </c:if>
          </c:if>
	    ...
	    <li ><a href="enAction.do?action=toIndexPage" >ENGLISH
